...
module.exports = {
    ...
    async InsertBlock(loginType, uid, uuid, name, description, date) {
        try {
            await this.query('INSERT INTO blocks (`login_type`, `uid`, `uuid`, `name`, `description`, `date`) VALUES (?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? DAY))', [loginType, uid, uuid, name, description, date])
            return true
        } catch (e) {
            console.error(e)
        }
    ...
    async UpdateUser(user) {
        try {
            await this.query('UPDATE users SET `uuid` = ?, `level` = ?, `exp` = ?, `coin` = ?, `cash` = ?, `point` = ?, `kill` = ?, `death` = ?, `assist` = ?, `blast` = ?, `rescue` = ?, `rescue_combo` = ?, `survive` = ?, `escape` = ?, `dead_graphics` = ?, `pure_graphics` = ?, `memo` = ?, `last_chat` = ? WHERE `id` = ?', [
                user.verify.uuid,
                user.level,
                user.exp,
                user.coin,
