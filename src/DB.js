...
module.exports = {
    ...
    async UpdateUser(user) {
        try {
            await this.query('UPDATE users SET `uuid` = ?, `level` = ?, `exp` = ?, `coin` = ?, `cash` = ?, `point` = ?, `win` = ?, `lose` = ?, `escape` = ?, `rankWin` = ?, `rankLose` = ?, `rankEscape` = ?, `dead_graphics` = ?, `pure_graphics` = ?, `memo` = ?, `last_chat` = ? WHERE `id` = ?', [
                user.verify.uuid,
                user.level,
                user.exp,
                user.coin,
